...
\item Extend GRModtool CLI to provide support for the block header parser. Make GRModtool use this code itself instead of the built-in code.
...
\\
Another GRModtool command can be called from CLI, for example from \texttt{gr\_modtool parse} for this purpose.
\\
...
\\ \\
Manual syntax check as shown below (code snippet from \texttt{parser\_cc\_block.py}) is no longer required as earlier done while parsing C++ files because both parsers throw compiler exceptions in case of syntax error (if any).
\begin{lstlisting}
...
June 24 - July 1 & Submit the code for Phase 1 evaluation and write a script to edit the optional YAML keys. \endlr
...
\item July 22: Merge code for complete parsing tool (Using pygccxml) integrated with GRModtool, and parsing AST with libclang.
