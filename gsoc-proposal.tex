...
GNU Radio's consists of vast library of blocks, both in-tree and out-of-tree. Presently, it is hard to use these blocks in a different context than GNU Radio itself, or interface with blocks automatically or programmatically. So, there is a need for python based tool that can interact with block headers written in C++, to automatically parse them and extract information about them such as which getters/setters they have, I/O signatures, factory signatures etc.
\\ \\
...
\\
GRModtool currently provides us with a powerful CLI to work with and so could be easily used for our purpose because of parsing tool itself being a CLI. GRModtool can be extended with the parsing tool as one of its utility.
\\ \\
The best python based parsers currently available and are used to parse of C++ code are libclang(with python bindings) and pygccxml, so they will be used to parse GNU Radio block header files.
...
\item Extend GRModtool CLI to provide support for this tool (block header parser). Make GRModtool use this code itself instead of the built-in code.
\item Parse the abstract representation, get the most information from them, create YAML files for GRC, write a script to change or add new optional YAML key parameters.
\item Extend its utility to take a list of files as input, or even a whole directory and parse them and also extend it to parse different file types (.cc), with this tool deciding how to manage each data-file type by its own, even  (if time permits)
\end{enumerate}

\section{Proposed Workflow}
\label{sec:theory}
Initially, I will be doing the groundwork by setting up the abstract implementation of the parser(pygccxml). Then I will be working on parsing the abstract representation and storing information in a python dictionary. Then, I will be creating YAML files for the GRC and integrating the tool with GRModtool. Same procedure will be followed for libclang
\\ \\
The working prototype of entire project (using pygccxml parser only) is available \href{https://github.com/aru31/parser-prototype}{here}. The file \href{https://github.com/aru31/parser-prototype/blob/master/ast_generator.py}{ast\_generator.py} implements the pygccxml parser to generate the abstract representation and the file \href{https://github.com/aru31/parser-prototype/blob/master/text_parser.py}{text\_parser.py} parses the entire AST to get all the useful information. All the data that we get using the function in the latter file is then returned in a python dictionary and a YAML file is created. \href{https://github.com/aru31/parser-prototype/blob/master/pygccxml_ast.txt}{pygccxml\_ast.txt} is AST of \href{https://github.com/aru31/parser-prototype/blob/master/sample.h}{sample.h} header file.
\subsection{Implementation of parsing tools}
GNU Radio API (headers) requires to use C++ for compiling, not straight C, which requires us to use parser to parse C++. Libclang(python-based) and pygccxml both do an excellent job for this purpose and are especially good for GNU Radio header files because of their brilliant, elegant yet simple and most importantly their generic structure.
\\
But there is a trade-off in their utility and which of them can be described as an ideal parser:-
\begin{itemize}
\item Pygccxml takes a good amount of computation time whereas libclang algorithms are better for this particular case.
\item Pygccxml is quite mature python package and also has a proper documentation which gives it an advantage over libclang. Libclang has a very small documentation.
...
\item But still libclang is a very popular C++ parser due to it's well-written source code and is under continuous development which provides an excellent opportunity to explore it.
\end{itemize}
So, it is worth it to use both of the python packages to parse the header files (with a choice given to user to use any one of them). Both the parsers can be abstractly implemented and either one of them could be used.
\subsubsection{Implementation of pygccxml}
Pygccxml provides a simple framework to navigate C++ declarations, using Python classes. It just needs to get access to the namespace in the file that we require to parse, and we are good to go. It automatically works its way down the hierarchy of the code structure parsing all the variables, function calls and so on. It is compatible with Python 2.7, 3.4, 3.5, pypy and pypy3.
\\ \\
...
It is due to the weird syntax of C++, all the current C++ parsers are still under constant development, thus need for their abstract implementation. Also the trade-off between the utility od both the parsers mentioned in section 2.1 makes it necessary to do so. Abstracting the details of implementation of both the parsers will allow us to work on both of them easily without complicating the code and we can even replace them with some other parser if need be.
\\ \\
...
\end{lstlisting}
 ...
 \begin{itemize}
\item \href{https://github.com/aru31/GSoC-Proposal/blob/master/AST/clang-ast.txt}{Clang-generated-AST}
\item \href{https://github.com/aru31/GSoC-Proposal/blob/master/AST/pygccxml-ast.txt}{Pygccxml-generated-AST}
 \end{itemize}
...
\\
Another GRModtool command can be called from CLI, for example from `gr\_modtool parse` for this purpose.
\\
...
\\ \\
The necessary documentation will be done in parallel with the development. There is a 13-week long coding period. I have my holidays in the months of June, July and August, so I'll work full time during this period, i.e., around 35-40 hours a week. I have made my deliverables accordingly on a weekly basis. 
\\ \\
...
June 24 - July 1 & Write a script to edit the optional YAML keys. \endlr
...
July 22 - July 29 & Complete the implementation on parsing AST generated by libclang with thorough testing. \endlr
July 16 - Aug 5 & Complete the remaining tasks for this parser(would be easy because most of the work is already completed earlier while fully implementing pygccxml).\endlr
...
\item Phase-2: Extend the support of GRModtool with the parsing tool and complete the first parser with thorough testing, complete parsing AST with libclang. 
...
\item July 22: Merge code for extended GRModtool utility and parsing AST with libclang.
...
\section{Conclusion}
Block header parsing tool is a great addition to the set of tools for GNU Radio that works independently of the whole code. Block headers being the most direct source of information of GNU Radio blocks, so parsing them directly from the source code itself is of great utility further.
